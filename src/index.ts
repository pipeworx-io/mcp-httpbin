...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'httpbin');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'get',
    description: 'Send a GET request to httpbin.org and receive back the echoed request: query parameters (query object), custom headers, origin IP, and User-Agent. Use for HTTP debugging.',
    inputSchema: { type: 'object', properties: { query: { type: 'object' }, headers: { type: 'object' } } },
  },
  {
    name: 'post',
    description: 'Send a POST request to httpbin.org with optional body and custom headers; returns the echoed request including parsed JSON body, form data, and headers.',
    inputSchema: {
      type: 'object',
  ...
  { name: 'json_anything', description: 'Fetch a sample JSON response from httpbin.org (/json). Use to verify JSON content-type handling or as a placeholder in tests.', inputSchema: { type: 'object', properties: {} } },
  { name: 'xml_anything', description: 'Fetch a sample XML response from httpbin.org (/xml). Use to verify XML content-type handling or as a placeholder in tests.', inputSchema: { type: 'object', properties: {} } },
  { name: 'html_anything', description: 'Fetch a sample HTML response from httpbin.org (/html). Use to verify HTML content-type handling or as a placeholder in tests.', inputSchema: { type: 'object', properties: {} } },
  { name: 'robots_txt', description: 'Fetch a sample robots.txt from httpbin.org (/robots.txt). Use to test robots.txt parsing or as a content-type placeholder.', inputSchema: { type: 'object', properties: {} } },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  const get = async (url: string, accept = 'application/json') => {
    ...
    // Without this check an httpbin outage came back as a SUCCESSFUL call whose
    // `body` was an HTML error page — so `ip`, which promises an object with an
    // `origin`, would hand back "<html>…503 Service Temporarily Unavailable…"
    // as a string and call it a win. The sibling `get`/`post` cases below have
    // always thrown on !res.ok; only this shared helper was missing it, which
    // is why the nightly schema check goes red whenever httpbin.org wobbles
    // instead of the outage being reported as an outage.
    if (!res.ok) throw await httpError(res, 'upstream_down: httpbin');
  ...
  switch (name) {
    case 'get': {
      ...
      if (!res.ok) throw await httpError(res, 'httpbin');
      return res.json();
    }
    case 'post': {
      ...
      const res = await pwFetch(`${BASE}/post`, { method: 'POST', headers, body });
      if (!res.ok) throw await httpError(res, 'httpbin');
      return res.json();
    }
    case 'headers':
    ...
    case 'status': {
      ...
      const res = await pwFetch(`${BASE}/status/${code}`, { headers: { 'User-Agent': UA } });
